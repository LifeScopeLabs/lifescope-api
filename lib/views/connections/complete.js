...
async function completeLogin(req, res, next) {
	...
	if (req.user == null) {
		try {
			...
			console.log('BitScoop Connection ID for new Connection: ' + bitscoopConnection.id);
...
async function completeConnection(req, res, next, user) {
	...
	let filter = {
		...
		remote_connection_id_string: req.query.existing_connection_id || req.query.connection_id,
	...
	if (connection) {
		console.log('Clearing duplicate connection');
		...
		console.log('Getting newly created connection');
		connection = await ConnectionTC.getResolver('findOne').resolve({
			args: {
				filter: {
					remote_connection_id_string: req.query.connection_id
				}
	...
	let map = await bitscoop.getMap(req.query.map_id);

	connection.provider = _.assign(provider, map);
	...
	if (req.query.existing_connection_id) {
		$set.remote_connection_id_string = req.query.existing_connection_id;
	}
